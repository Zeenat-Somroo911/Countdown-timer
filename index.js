...
async function fastAnimate(text, delay = 18) {
    for (let char of text) {
        process.stdout.write(char);
        await new Promise((resolve) => setTimeout(resolve, delay));
    }
}
async function slowAnimate(text, delay = 30) {
    for (let char of text) {
        process.stdout.write(char);
        await new Promise((resolve) => setTimeout(resolve, delay));
    }
...
let appName = chalk.magenta.underline.italic(` Welcome "${userName}" in Countdown Timer App: `);
